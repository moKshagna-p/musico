...
export const useApiClient = () => {
  ...
  const getFeaturedReleases = async (limit = 24, options = {}) => {
  ...
  const getRecentPopularReleases = async (limit = 24, options = {}) => {
    const forceRefresh = Boolean(options?.forceRefresh)
  ...
  const prefetchReleaseDetails = async (releaseId) => {
    try {
